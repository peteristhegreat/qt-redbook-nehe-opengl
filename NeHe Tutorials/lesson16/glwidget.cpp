...
// Constructor
GLWidget::GLWidget() {
    setWindowTitle("NeHe's OpenGL Lesson 16");
    time = QTime::currentTime();
    timer = new QTimer(this);
    timer->setSingleShot(true);
    connect(timer, SIGNAL(timeout()), this, SLOT(updateGL()));
    filter = 2;
    lightChanged = filterChanged = fogChanged = false;
    changeAmount = 0;
    ...
    fogMode[1] = GL_EXP2;
    fogMode[2] = GL_LINEAR;
    fogModeName[0] = "GL_EXP";
    fogModeName[1] = "GL_EXP2";
    fogModeName[2] = "GL_LINEAR";
...
void GLWidget::initializeGL() {
    ...
    glClearColor(0.5f, 0.5f, 0.5f, 1.0f); // Set the clear color to the fog color
...
void GLWidget::paintGL() {
    ...
    glLoadIdentity();
    if ((lightChanged || filterChanged || fogChanged) && changeAmount > 0) {
        QString str;
        if (filterChanged)
            ...
            str = QString("Fogmode is now ") + fogModeName[fogFilter];
        QFont font;
